...
def main():
    ...
    """plt.figure()
    src_dots = plt.imshow(src_img)
    plt.title('Src & dots')
    plt.plot(match_p_src[0], match_p_src[1],'bo')
    plt.show()
    plt.figure()
    dst_dots = plt.imshow(dst_img)
    plt.title('Dst & dots')
    plt.plot(match_p_dst[0], match_p_dst[1],'bo')
    plt.show()"""
    ...
    print([fit_percent, dist_mse, max_err])
    input()
    """
    ...
    plt.figure()
    forward_panorama_imperfect_matches_plot = plt.imshow(transformed_image_fast)
    plt.title('Forward Panorama imperfect matches with RANSAC')
    plt.show()
